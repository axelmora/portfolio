...
format:
  revealjs:
    ...
    logo: wbc_logo.png
    css: styles.css
    footer: "World Baseball Classic 2026 | Análisis de Datos"
    transition: slide
...
nation_ids <- read_csv("nation_ids.csv", show_col_types = FALSE)

all_rosters_full <- all_rosters_full %>%
  left_join(nation_ids %>% select(team_name, flag), by = "team_name")

teams <- sort(unique(all_rosters_full$team_name))
```
